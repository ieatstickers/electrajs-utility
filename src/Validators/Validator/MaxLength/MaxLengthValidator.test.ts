...
describe("MaxLengthValidator", () => {
  ...
  describe("validate", () => {
    ...
    it("returns correct response when validating a value of valid type but invalid length", () => {
      ...
      expect(message).toBe("Value must be no more than 5 in length - string of length 7 provided");
      
      const testValue2 = [ 1, 2, 3, 4, 5, 6 ];
      const { value: validatorValue2, valid: valid2, message: message2 } = validator.validate(testValue2);
      expect(validatorValue2).toBe(testValue2);
      expect(valid2).toBe(false);
      expect(message2).toBe("Value must be no more than 5 in length - array of length 6 provided");
    });
    ...
    it("returns correct response when validating a null value (optional)", () => {
      ...
      {
        const validator = new MaxLengthValidator(5, { optional: true });
    ...
    it("returns correct response when validating a null value (not optional)", () => {
      ...
      for (const type in nullValues)
      {
        ...
        const validator = new MaxLengthValidator(5, { optional: false });
        const { value: validatorValue, valid, message } = validator.validate(nullValue);
        expect(validatorValue).toBe(nullValue);
        expect(valid).toBe(false);
    ...
    it("throws error when given an invalid value and throwErrors is true", () => {
      const invalidValues = {
        ...
        array: {
          value: [ 1, 2, 3, 4, 5, 6, 7],
          valid: false,
          message: "Value must be no more than 5 in length - array of length 7 provided"
        },
        string: {
          value: "testing",
          valid: false,
          message: "Value must be no more than 5 in length - string of length 7 provided"
        }
      }
      
      for (const { value, valid, message } of Object.values(invalidValues))
      {
        const validator = new MaxLengthValidator(5, { throwErrors: true });
